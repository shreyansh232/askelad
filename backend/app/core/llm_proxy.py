...
class LLMProxy:
    ...
    def _message_from_provider(self, message: Any) -> ChatMessage:
        return ChatMessage(
            ...
            tool_calls=self._normalize_tool_calls(
                self._get_attr(message, "tool_calls")
            ),
        )
    ...
    async def _collect_stream(
        ...
        async for chunk in stream:
            ...
            for position, tool_call_delta in enumerate(
                self._get_attr(delta, "tool_calls", []) or []
            ):
                index = self._get_attr(tool_call_delta, "index", position)
                current = tool_calls.setdefault(index, ToolCall())

                # Only update fields if they are present and not null in the delta
                new_id = self._get_attr(tool_call_delta, "id")
                if new_id:
                    current.id = new_id

                new_type = self._get_attr(tool_call_delta, "type")
                if new_type:
                ...
                if function_delta:
                    current.function.name += (
                        self._get_attr(function_delta, "name") or ""
                    )
                    current.function.arguments += (
                        self._get_attr(function_delta, "arguments") or ""
                    )
